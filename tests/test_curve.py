...
def test_curve_in_contract(w3, coins, yerc20s, swap, n_coins):
    ...
    for c, cc, u in zip(coins, yerc20s, UU):
        ...
        rate = int(cc.caller.get_price_per_full_share() * (1 + 0.1 * len(n_ccoins)))
    ...
    for k in range(5):
        for i, j in permutations(range(3), 2):
            ...
            rate_x = yerc20s[i].caller.get_price_per_full_share()
            ...
            rate_y = yerc20s[j].caller.get_price_per_full_share()
