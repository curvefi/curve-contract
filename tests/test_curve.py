...
def test_curve_in_contract(w3, coins, cerc20s, swap, n_coins):
    ...
    for c, cc, u, l in zip(coins, cerc20s, UU, use_lending):
        ...
        if l:
            rate = int(cc.caller.exchangeRateCurrent() * (1 + 0.1 * len(n_ccoins)))
    ...
    rates = [int(c.caller.exchangeRateCurrent()) * p if l else 10 ** 18 * p
    ...
    for k in range(5):
        for i, j in permutations(range(3), 2):
            ...
            rate_x = cerc20s[i].caller.exchangeRateCurrent() if use_lending[i] else 10 ** 18
            ...
            rate_y = cerc20s[j].caller.exchangeRateCurrent() if use_lending[j] else 10 ** 18
