...
def test_add_liquidity(w3, coins, swap):
    ...
    swap.functions.add_liquidity(0, 100 * U, 110 * U, int(time.time()) + 3600).\
        transact({'from': w3.eth.accounts[0]})
    ...
    with pytest.raises(TransactionFailed):
        ...
        swap.functions.\
            add_liquidity(0, 100 * U, 99 * U, int(time.time()) + 3600).\
            transact({'from': w3.eth.accounts[0]})

    with pytest.raises(TransactionFailed):
        # Fail because transaction is expired
        swap.functions.\
            add_liquidity(0, 100 * U, 110 * U, int(time.time()) - 3600).\
            transact({'from': w3.eth.accounts[0]})
