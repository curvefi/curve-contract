...
def __init__(
    ...
    @param _coins List of all ERC20 contract addresses of coins
    ...
    for i in range(N_COINS):
        rates[i] = 10 ** (36 - ERC20(_coins[i]).decimals())
...
def _xp_mem(_balances: uint256[N_COINS]) -> uint256[N_COINS]:
    ...
    for i in range(N_COINS):
        result[i] = unsafe_div(RATE_MULTIPLIERS[i] * _balances[i], PRECISION)
    return result
...
def get_D(_xp: uint256[N_COINS], _amp: uint256) -> uint256:
    ...
    """
    S: uint256 = 0
    any_x_is_zero: uint256 = 0
    Dprev: uint256 = 0
    for x in _xp:
        S += x
        any_x_is_zero = bitwise_or(any_x_is_zero, x)
    if S == 0:
        return 0
    assert any_x_is_zero > 0  # if it is 0, this will be borked: only withdrawal will work. And that is good
    ...
    for i in range(255):
        D_P: uint256 = D
        for x in _xp:
            D_P = unsafe_div(D_P * D, x * N_COINS)
        Dprev = D
        D = (unsafe_div(Ann * S, A_PRECISION) + D_P * N_COINS) * D / (unsafe_div((Ann - A_PRECISION) * D, A_PRECISION) + unsafe_add(N_COINS, 1) * D_P)
        # Equality with the precision of 1
        # -1 <= D - Dprev <= 1
        # 0 <= D + 1 - Dprev <= 2
        if unsafe_sub(unsafe_add(D, 1), Dprev) <= 2:
            return D
...
def add_liquidity(
    ...
    if total_supply > 0:
        ...
        # self.fee * N_COINS / (4 * (N_COINS - 1))
        base_fee: uint256 = unsafe_div(unsafe_mul(self.fee, N_COINS), 4 * unsafe_sub(N_COINS, 1))
        for i in range(N_COINS):
            ...
            self.balances[i] = new_balance - unsafe_div(fees[i] * ADMIN_FEE, FEE_DENOMINATOR)
...
def get_y(i: int128, j: int128, x: uint256, xp: uint256[N_COINS]) -> uint256:
    ...
    j_256: uint256 = convert(j, uint256)  # dev: j below zero
    assert j_256 < N_COINS  # dev: j above N_COINS

    # should be unreachable, but good for safety
    i_256: uint256 = convert(i, uint256)  # dev: i below zero
    assert i_256 < N_COINS  # dev: i above N_COINS
    ...
    for _i in range(255):
        ...
        # -1 <= y - y_prev <= 1
        # 0 <= y + 1 - y_prev <= 2
        if unsafe_sub(unsafe_add(y, 1), y_prev) <= 2:
            return y
    raise
...
def get_dy(i: int128, j: int128, dx: uint256) -> uint256:
    ...
    x: uint256 = xp[i] + unsafe_div(dx * RATE_MULTIPLIERS[i], PRECISION)
    y: uint256 = self.get_y(i, j, x, xp)
    dy: uint256 = xp[j] - y - 1
    fee: uint256 = unsafe_div(self.fee * dy, FEE_DENOMINATOR)
    return unsafe_div((dy - fee) * PRECISION, RATE_MULTIPLIERS[j])
...
def exchange(
    ...
    x: uint256 = xp[i] + unsafe_div(_dx * RATE_MULTIPLIERS[i], PRECISION)
    y: uint256 = self.get_y(i, j, x, xp)

    dy: uint256 = xp[j] - y - 1  # -1 just in case there were some rounding errors
    dy_fee: uint256 = unsafe_div(dy * self.fee, FEE_DENOMINATOR)

    # Convert all to real units
    dy = unsafe_sub(dy, dy_fee) * PRECISION / RATE_MULTIPLIERS[j]
    assert dy >= _min_dy, "Exchange resulted in fewer coins than expected"

    dy_admin_fee: uint256 = unsafe_div(dy_fee * ADMIN_FEE, FEE_DENOMINATOR)
    dy_admin_fee = unsafe_div(dy_admin_fee * PRECISION, RATE_MULTIPLIERS[j])
...
def _calc_withdraw_one_coin(_burn_amount: uint256, i: int128) -> uint256[2]:
    ...
    dy_0: uint256 = unsafe_div((xp[i] - new_y) * PRECISION, RATE_MULTIPLIERS[i])  # w/o fees
    dy = unsafe_div((dy - 1) * PRECISION, RATE_MULTIPLIERS[i])  # Withdraw less to account for rounding errors
...
def remove_liquidity_one_coin(
    ...
    self.balances[i] -= unsafe_div(dy[0] + dy[1] * ADMIN_FEE, FEE_DENOMINATOR)
...
def admin_balances(i: uint256) -> uint256:
    return ERC20(COINS[i]).balanceOf(self) - self.balances[i]
