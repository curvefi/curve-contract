...
def test_remove_balanced(bob, swap, underlying_coins, wrapped_coins, pool_token, divisor, initial_amounts, n_coins, base_amount):
...
def test_remove_some(bob, swap, underlying_coins, wrapped_coins, pool_token, idx, initial_amounts, n_coins, base_amount):
...
@pytest.mark.itercoins("idx")
def test_remove_one(bob, swap, underlying_coins, wrapped_coins, pool_token, idx, initial_amounts, base_amount, n_coins):
    amounts = [0] * n_coins
...
def test_exceed_max_burn(bob, swap, wrapped_coins, pool_token, divisor, initial_amounts, base_amount, n_coins):
    amounts = [i // divisor for i in initial_amounts]
    max_burn = (n_coins * 10**18 * base_amount) // divisor
