...
def test_remove_balanced(bob, swap, underlying_coins, wrapped_coins, pool_token, divisor, initial_amounts, initial_amounts_underlying, n_coins, base_amount):
    ...
    for i, coin in enumerate(wrapped_coins):
        ...
        assert coin.balanceOf(swap) == initial_amounts[i] - (amounts[i] * 10**18 // 10**underlying_coins[i].decimals())
...
def test_remove_some(bob, swap, underlying_coins, wrapped_coins, pool_token, idx, initial_amounts, initial_amounts_underlying, n_coins, base_amount):
...
@pytest.mark.itercoins("idx")
def test_remove_one(bob, swap, underlying_coins, wrapped_coins, pool_token, idx, initial_amounts, initial_amounts_underlying, base_amount, n_coins):
    amounts = [0] * n_coins
    amounts[idx] = initial_amounts_underlying[idx] // 2
...
def test_exceed_max_burn(bob, swap, wrapped_coins, pool_token, divisor, initial_amounts_underlying, base_amount, n_coins):
    amounts = [i // divisor for i in initial_amounts_underlying]
    max_burn = (n_coins * 10**18 * base_amount) // divisor
...
def test_events(bob, swap, pool_token, initial_amounts, initial_amounts_underlying):
    amounts = [i // 2 for i in initial_amounts_underlying]
    tx = swap.remove_liquidity_imbalance(amounts, 2**256 - 1, True, {'from': bob})

    event = tx.events["RemoveLiquidityImbalance"]
    assert event["provider"] == bob
    assert event["token_amounts"] == [i // 2 for i in initial_amounts]
