...
def test_curve_in_contract(w3, coins, yerc20s, swap, n_coins):
    ...
    curve = Curve(2 * 360, balances, 2, rates)

    for k in range(5):
        for i, j in permutations(range(2), 2):
            dx = random.randrange(2 * n_coins * UU[i])
            rate_x = yerc20s[i].caller.getPricePerFullShare() if use_lending[i] else 10 ** 18
            ...
            rate_y = yerc20s[j].caller.getPricePerFullShare() if use_lending[j] else 10 ** 18
