...
def test_remove_liquidity_balanced(alice, swap, coins, pool_token, divisor):
    ...
    assert pool_token.balanceOf(alice) == (N_COINS * 10**24) - max_burn
    assert pool_token.totalSupply() == (N_COINS * 10**24) - max_burn
...
def test_remove_two_coins(alice, swap, coins, pool_token, idx):
    ...
    ideal_balance = int(N_COINS * 10 ** 24 * (1 - (N_COINS - 1) / N_COINS / 2))
...
def test_remove_one_coin(alice, swap, coins, pool_token, idx):
    ...
    assert abs(pool_token.balanceOf(alice) - burn_amount)/burn_amount < 1e-9
