...
def test_add_liquidity(
    ...
    swap.add_liquidity(initial_amounts_underlying, 0, True, {'from': bob})
...
def test_add_liquidity_with_slippage(bob, swap, pool_token, underlying_decimals, n_coins):
...
def test_add_one_coin(
    bob, swap, underlying_coins, wrapped_coins, pool_token, initial_amounts, initial_amounts_underlying, base_amount, idx, n_coins
):
    amounts = [0] * n_coins
    amounts[idx] = initial_amounts_underlying[idx]
    swap.add_liquidity(amounts, 0, True, {'from': bob})

    for i, coin in enumerate(underlying_coins):
        assert coin.balanceOf(bob) == initial_amounts_underlying[i] - amounts[i]
        assert coin.balanceOf(swap) == 0

    for i, coin in enumerate(wrapped_coins):
        assert coin.balanceOf(bob) == initial_amounts[i]
        assert coin.balanceOf(swap) == initial_amounts[i] * (2 if idx == i else 1)
...
def test_event(bob, swap, pool_token, initial_amounts, initial_amounts_underlying):
    tx = swap.add_liquidity(initial_amounts_underlying, 0, True, {'from': bob})
